...
<html>
	<head>
		...
		<style>
			...
			#btn_confirm {width:auto; background-color:#009E94;margin-left:-6px}
  		</style>
				...
				layui.use('table', function(){
					...
					table.render({
						...
						cols : [ [ //表头
	    					...
	    					{field : 'check', title: '操作', toolbar:"#barDemo", align: 'center'}
						] ]
				    });
					...
					table.on('tool(check_filter)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
			        	...
			        	if(layEvent === 'confirm'){ //查看
			        		$.ajax({
			        		url: '<%=request.getContextPath() %>/BackEnd/order/confirmCheck',
			        			type: 'post',
			        			data: {'oid': data.oid},
			        			dataType: 'json',
				...
				<div class="layui-card-body">
					<form id="search_form" class= "scree" method="post">
						...
						<select id="status" name="status" disabled="true">
							<option value="paid" selected="true">已付款</option>
						</select>
					...
					<div class="layui-form layui-border-box layui-table-view" style="height: 450px; " >
					    <div class="layui-table-box">
							<table id="check_table" lay-filter="check_filter"></table>
